...
if($deleteAllEnvironments -eq $True) {
    Write-Host "Querying all environments in Space: $spaceName"
    $environmentsResource = Invoke-RestMethod -Uri "$octopusURL/api/$($space.Id)/environments?skip=0&take=100000" -Headers $header   
    $environmentNames = $environmentsResource.Items | ForEach-Object { "$($_.Name)" }
}

Write-Host "Found $($environmentNames.Length) environments"
